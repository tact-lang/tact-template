...
import { beginCell, Dictionary, type Cell, type DictionaryValue } from "@ton/core";
...
export function setStoragePrices(configRaw: Cell, prices: StorageValue): BlockchainConfig {
    const config = configRaw
        .beginParse()
        .loadDictDirect(Dictionary.Keys.Int(ConfigKeyLength), Dictionary.Values.Cell());
    ...
    config.set(ConfigStoragePriceIndex, beginCell().storeDictDirect(storageData).endCell());
